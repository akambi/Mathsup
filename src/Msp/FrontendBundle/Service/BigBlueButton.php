...
namespace Msp\FrontendBundle\Service;

use Symfony\Component\DependencyInjection\SimpleXMLElement;
/**
 * Description of BigBlueButtonApi
 *
 * @author arnaud
 */
class BigBlueButton {
    ...
    public function bbb_wrap_simplexml_load_file($url){	
    ...
    */
    public function createMeetingAndGetJoinURL( $username, $meetingID, $meetingName, $welcomeString, $mPW, $aPW, $SALT, $URL, $logoutURL, $record='false', $duration=0, $voiceBridge=0, $metadata = array() ) {
            ...
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getCreateMeetingURL($meetingName, $meetingID, $aPW, $mPW, $welcomeString, $logoutURL, $SALT, $URL, $record, $duration, $voiceBridge, $metadata ) );
    ...
    */
    public function getMeetingInfo( $meetingID, $modPW, $URL, $SALT ) {
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getMeetingInfoURL( $meetingID, $modPW, $URL, $SALT ) );
            if($xml){
    ...
    */
    public function getMeetings( $URL, $SALT ) {
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getMeetingsURL( $URL, $SALT ) );
            if( $xml && $xml->returncode == 'SUCCESS' ) {
    ...
    */
    ...
    $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getRecordingsURL( $meetingID, $URL, $SALT ) );
    if( $xml && $xml->returncode == 'SUCCESS' && $xml->messageKey ) {//The meetings were returned
        return array('returncode' => (string)$xml->returncode, 'message' => (string)$xml->message, 'messageKey' => (string)$xml->messageKey);
    } else if($xml && $xml->returncode == 'SUCCESS'){ //If there were meetings already created
        $recordings = array();
    ...
    */
    public function endMeeting( $meetingID, $modPW, $URL, $SALT ) {
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getEndMeetingURL( $meetingID, $modPW, $URL, $SALT ) );
    ...
    public function doDeleteRecordings( $recordIDs, $URL, $SALT ) {
        ...
        foreach( $ids as $id){
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getDeleteRecordingsURL($id, $URL, $SALT) );
            if( $xml && $xml->returncode != 'SUCCESS' )
                return false;
        }
        return true;
    ...
    public function doPublishRecordings( $recordIDs, $set, $URL, $SALT ) {
        $ids = 	explode(",", $recordIDs);
        foreach( $ids as $id){
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getPublishRecordingsURL($id, $set, $URL, $SALT) );
            if( $xml && $xml->returncode != 'SUCCESS' )
                return false;
        }
        return true;
    ...
    public function getServerVersion( $URL ){
        ...
        $xml = $this->bbb_wrap_simplexml_load_file( $base_url_record );
        if( $xml && $xml->returncode == 'SUCCESS' )
            return $xml->version;
        else
            return NULL;
    ...
    */
    public function isMeetingRunning( $meetingID, $URL, $SALT ) {
            $xml = $this->bbb_wrap_simplexml_load_file( BigBlueButton::getIsMeetingRunningURL( $meetingID, $URL, $SALT ) );
            if( $xml && $xml->returncode == 'SUCCESS' ) 
    ...
    */
    ...
    public function bigbluebutton_display_redirect_script($bigbluebutton_joinURL, $meetingID, $meetingName, $name)
    {
